...
void update_motor_speeds(float left_rpm, float right_rpm, float dt) {
    ...
    if (on_black_R) {
        ...
        //left_pid_output = 65 + black_turn_speed;
        //right_pid_output = 50 - black_turn_speed;
        left_pid_output = 100;
        right_pid_output = 35;
    }
    ...
    if (on_black_L) {
        ...
        //left_pid_output = 50 - black_turn_speed;
        //right_pid_output = 65 + black_turn_speed;
        left_pid_output = 35;
        right_pid_output = 100;
    }
...
void auto_mode_task(void *pvParameters) {
    ...
    while (true) {
        if (auto_mode) {
            if (!in_cooldown && !stopped && !on_black_R && !on_black_L && !emergency_stop) {
...
void ir_sensor_init() {
    /* adc_init();
    adc_gpio_init(LINE_SENSOR_L_PIN);
    adc_select_input(0); */
    gpio_init(LINE_SENSOR_L_PIN);
    gpio_set_dir(LINE_SENSOR_L_PIN, GPIO_IN);
    gpio_pull_down(LINE_SENSOR_L_PIN);
 ...
 */
    ...
    gpio_init(LINE_SENSOR_R_PIN);
    gpio_set_dir(LINE_SENSOR_R_PIN, GPIO_IN);
    gpio_pull_down(LINE_SENSOR_R_PIN);
...
void line_following_task(void *pvParameters) {
    ...
    while (true) {
        ...
        if (line_following_mode) {
            if (gpio_get(LINE_SENSOR_L_PIN) == 0 && gpio_get(LINE_SENSOR_R_PIN) == 0) {
                on_black_R = false;
                on_black_L = false;
            ...
            else if (gpio_get(LINE_SENSOR_L_PIN) == 1 && gpio_get(LINE_SENSOR_R_PIN) == 0) {
                black_counter++;
                if (black_counter > 4) {
                    on_black_L = true;
                    on_black_R = false;
            ...
            else if (gpio_get(LINE_SENSOR_L_PIN) == 0 && gpio_get(LINE_SENSOR_R_PIN) == 1) {
                black_counter++;
                if (black_counter > 4) {
                    on_black_L = false;
                    on_black_R = true;
        ...
        /* if (line_following_mode) {
            if (gpio_get(LINE_SENSOR_L_PIN) == 0) {
                //printf("WHITE\n");
                on_black_R = false;
                target_speed = 60; // Set target speed for straight movement
            } else {
